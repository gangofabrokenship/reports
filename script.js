...
const CODEX_RULES = [
    ...
    },
    {
        ...
        points: 'Запрещено посещение дополнительных Сундуков (№1-3) без разрешения ответственного за Сундук.'
    },
    {
        rule: 'Посещение Каюты Кока и Боцмана',
        points: 'Запрещено выходить в статус «Спит».'
    },
    {
        rule: 'Посещение территории Эскадры',
        points: 'Запрещено выходить в статус «Спит» на Акульей арене, Мёртвом берегу и в Останках падших игрокам, не состоящим в отряде. Игроки, состоящие в отряде, могут выходить в статус «Спит» в указанных локациях, но только в 6 нижнем ряду, чтобы не создавать помех для активного бега; Запрещено посещение Черепа морского гиганта и последующих локаций игрокам, не состоящим в отряде.'
    },
    {
    ...
    },
    {
        ...
        points: 'Запрещено закапывать предметы, наносящие вред здоровью; Запрещено оставлять предметы, полученные во время охоты, на охотничьих локациях более чем на 2 часа. Дичь, пойманная во время мероприятий, должна быть убрана в соответствии с правилами мероприятия; Запрещено оставлять предметы, полученные во время ныряния, на Глубоководье более чем на 2 часа; Запрещено хранить предметы вне локаций, где это допустимо, или игнорировать условия их хранения.'
    },
    {
        rule: 'Игнорирование требований к надутым игрокам',
        points: 'Запрещено посещение лазательных и нырятельных локаций с открытой регистрацией; Запрещено уходить в статус «Спит» вне территории лагеря с открытой регистрацией; Запрещено посещение общих и официальных нейтральных земель, в том числе с закрытой регистрацией; Запрещено посещение территорий других племён, в том числе с закрытой регистрацией.'
    },
    {
        ...
        points: 'Запрещено без соответствия требованиям; Запрещено при наличии чёрных меток или иного наказания, ограничивающего выход с территории племени; Запрещено надутым игрокам.'
    },
    {
        rule: 'Игнорирование правил поведения на нейтральных землях и иных территориях за пределами шайки',
        points: 'Запрещено использование опасных боевых режимов (Т+2, Т+3); Запрещено посещение территории другой фракции без соответствующего разрешения; Запрещено нарушение правил сопровождения; Запрещено использование сыра Мурлычки; Запрещено уходить в статус «Спит» за пределами личных локаций или карманов — клеток, полностью окружённых переходами или игроками с ростом 100%; Запрещено посещение личных локаций игроков без соответствующего разрешения.'
    },
    {
        rule: 'Халатное отношение к обязанностям',
        points: 'Запрещено нарушение правил посещения деятельности; Запрещено нарушение правил проведения и выполнения деятельности, в том числе отрядной; Запрещено игнорирование обязанностей, предписанных должностью; правило действует на заместителей и глав отрядов, сфер и иных направлений, включая Верховную сферу.'
    },
    {
        rule: 'Взаимодействие с боевыми ботами',
        points: 'Запрещено взаимодействовать с боевыми ботами без выполнения требований; Запрещено взаимодействовать с ежемесячным ботом-убийцей вне отряда Крысоловов; Запрещено взаимодействовать с иноплеменными боевыми ботами без соблюдения условий, выставленных племенем, которое посещает игрок.'
    },
    {
        rule: 'Создание помех',
        points: 'Запрещено создание помех для игрового процесса.'
    }
];
...
// Обновление поля "Пункт"
function updateCodexPoint() {
    const sel = qs('codexRule');
    const pointEl = qs('codexPoint');
    if (!sel || !pointEl) return;
    const entry = CODEX_RULES.find(r => r.rule === sel.value);
    pointEl.value = (entry && entry.points) ? entry.points : '—';
}

if (qs('codexRule')) {
    fillCodexRules();
    qs('codexRule').onchange = updateCodexPoint;
    updateCodexPoint();
}
...
if (qs('codexGenerate')) {
    qs('codexGenerate').onclick = () => {
        ...
        const point = qs('codexPoint').value.trim() || '—';
